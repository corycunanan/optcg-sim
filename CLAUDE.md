...
**Exceptions to rule 1:** `src/components/ui/sidebar.tsx` (lines 132-137, 190-193, 611-614) sets inline `style={{ "--sidebar-width": ... }}` etc. to feed responsive width values into CSS custom properties. These are dynamic per-render values, not design-token colors/borders, and Tailwind has no utility for arbitrary `--var` assignment.
