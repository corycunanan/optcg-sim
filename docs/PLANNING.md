...
### 3. Card Data Sourcing

**Risk: Medium. Impact: High (blocks everything).**

The PRD lists HTML scraping from the official site as primary. But:
- Official site structure hasn't been confirmed as scrapable
- ToS hasn't been checked
- Community data repos (optcg-data on GitHub) haven't been evaluated for completeness, accuracy, or licensing
- Card images need a legal source

If the data pipeline doesn't work, there's no card database, no deck builder, no simulator.

**Mitigation:** Investigate this FIRST in M0 — before writing any app code. Evaluate community JSON repos as primary source. A few hours of research saves weeks of wasted work.
...
1. **Card data source** — spend a day evaluating community repos and the official site. Determine primary data source.
