...
### 3. Card Data Sourcing — ✅ RESOLVED

**Risk: Low (was Medium). Impact: High (blocks everything).**

**Resolution:** Identified `punk-records` (https://github.com/buhbbl/punk-records) as primary data source. Community dataset generated by the `vegapull` Rust scraper. Contains 4,007 unique cards across 50 English packs, generated March 8, 2026. Per-card JSON with all required fields (id, name, category, colors, cost, power, counter, attributes, types, effect text, trigger text, image URLs, rarity). Includes indexes for quick lookup.

The original plan to build a custom scraper or manually save HTML is no longer needed. The pipeline reduces to a JSON import script (punk-records → Prisma).

Remaining gaps: ban status, block rotation, art variants, and errata are not in the dataset and must be maintained manually.
