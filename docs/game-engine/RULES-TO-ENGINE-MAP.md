...
| **5-2-1-5-1.** "At the start of the game" effects | **IMPL** | `pregame.ts → advanceStartOfGameEffects()`; [`opt-476-start-of-game-effects.test.ts`](../../workers/game/src/__tests__/opt-476-start-of-game-effects.test.ts) | Authored Leader rule modifications resolve in priority-decider order and pause durably on prompts. `PRIORITY_ROLL` assigns the roll winner as priority decider even if they choose to play second; roll-skipping `HOST_FIRST`, `GUEST_FIRST`, and `RANDOM_FIXED` assign the configured or privately selected first player to both roles |
