...
`<Board>` is the only runtime consumer of `BoardLayout`; it passes the `<ScaledBoard>` design canvas and measured outer scale down so `BoardLayout` can fit its widened intrinsic board composition inside 1920×1080 without measuring the browser window itself. Gameplay card, hand, and DON footprints are sourced from the same board constants used by the geometry and animation layers, so changing the board scale changes the rendered pieces rather than only the empty grid spacing.
