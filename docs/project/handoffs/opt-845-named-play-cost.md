...
- Existing generic event cause matching only derives cause for `CARD_KO` (plus specialized rest handling), so effect-only removal/trash observer support needs its own source-based audit. The negative rule-trash test establishes no false activation, not complete support for all such watchers.
