...
last-updated: 2026-04-27 (project + tickets created; M5d merges before PR 1 begins)
---
...
|-------|--------|-------|----------|------------|--------|----|-------|
| 1 | [OPT-308](https://linear.app/optcg-sim/issue/OPT-308) | Build `<ScaledBoard>` primitive: viewport measurement, transform-scale, initial-render handling | 3 | — | Backlog | — | Gate ticket for PR 1. Establishes `BoardScaleContext`. Initial render: opacity-0 until first measure, fade-in respects reduced-motion. Resize re-scaling always snaps. |
...
**Next up:** OPT-308 — gate ticket for PR 1. Establishes `<ScaledBoard>` and `BoardScaleContext`. Until OPT-308 lands, OPT-310 / OPT-311 / OPT-312 are blocked, but OPT-309 / OPT-313 / OPT-319 are independent and can run in parallel.
