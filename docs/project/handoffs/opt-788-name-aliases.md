...
- [OPT-852](https://linear.app/optcg-sim/issue/OPT-852) tracks existing restricted-card validation checking each row against the one-copy limit. Variant aggregation for ban-list restrictions is outside this ticket's alias/four-copy criteria.
...
## Review correction: next-play cost filter

Independent review and external Codex thread `discussion_r4002773110` found one material consumer omitted from the initial shared-filter inventory: `modifiers.ts::matchesOneTimeFilter` compared printed names directly. It now uses the same intrinsic name predicate for cost quotation and modifier consumption. The external thread is dispositioned by this fix; no reply or thread-resolution action was taken.

OP12-061's authored discount is for the next hand [Trafalgar Law] costing four or more. New tests in `opt-810-authored-fidelity.test.ts` use EB04-038's real card ID, authored schema, printed cost six and power 8000 (card data cross-check: https://onepiece.limitlesstcg.com/cards/EB04-038). Two tests failed before the correction: quote six instead of four, and a legal four-DON!! play rejected as unaffordable. They now pass: an unrelated three-cost play pays three and leaves the discount unused, aliased EB04-038 pays four and consumes it, and the second copy quotes six. A separate four-ready-DON!! scenario proves action validation and payment accept exactly the discounted cost. Temporarily removing alias metadata only from the one-time matcher reproduces both failures; restored code passes the 40 related tests.

The reviewer also identified separate legacy exact-name cost handlers `REST_NAMED_CARD` (Uta/Enel) and `TRASH_NAMED_CARD_FROM_HAND_OR_STAGE` (Ark Noah). Their current authored targets do not intersect any of the eight alias cards. This is a latent extension limitation, not a demonstrated current card defect or a scope expansion. The current official rules v1.2.1 (August 28, 2026), independently checked by the reviewer, retain the relevant alias/card-number clauses from the repository's supported v1.2.0; no rules-version migration is included.
