...
| 42 | OPT-487 | RETURN_TO_DECK silently ignores non-Character source zones | — | OPT-474, OPT-475 | In Review | [#305](https://github.com/corycunanan/optcg-sim/pull/305) | Canonical returns and private movement events for every legal source; field replacements remain field-only. |
...
### OPT-478 → OPT-479
**From:** session on 2026-07-12 · **Commit:** `bce6adb` · **PR:** [#298](https://github.com/corycunanan/optcg-sim/pull/298)

- **Primer:** Resolver recursion now receives one frozen, typed, non-serialized `EffectResolverServices` bundle. Replacement execution requires the service at every direct and resume boundary; the nullable module-global callbacks and fail-open dispatcher path are gone. `cost-handler.ts` is a 20-line compatibility façade over six focused, acyclic cost modules.
- **Read first:** `workers/game/src/engine/effect-resolver/services.ts`, `resolver.ts`, `replacements.ts`, `cost-handler.ts`, the `cost/` directory, and `workers/game/src/__tests__/opt-478-resolver-architecture.test.ts`; then begin OPT-479 at `workers/game/src/GameSession.ts`.
...
### OPT-487 → OPT-480
**From:** session on 2026-07-13 · **Commit:** `130e9f2` · **PR:** [#305](https://github.com/corycunanan/optcg-sim/pull/305)

- **Primer:** `RETURN_TO_DECK` now commits every canonical cross-zone transition instead of discarding non-Character results. Removal and leave-field replacements explicitly match only Character/Stage targets, so Trash, Hand, and Life returns cannot spend field replacements.
- **Read first:** `workers/game/src/engine/effect-resolver/card-mutations.ts`, `workers/game/src/engine/replacements.ts`, and `workers/game/src/__tests__/opt-487-return-to-deck-source-zones.test.ts`; then follow OPT-479's architecture guard into the runtime unions and duplicate resolver owned by OPT-480.
- **Gotchas / do NOT touch:** Same-deck result-reference placement remains an identity-preserving reorder in `actions/removal.ts`. Keep old/new destination identities owner-only in movement events, and retain Character/Stage prohibition and replacement behavior while tightening types.
...
- **Pointer:** PR #305; inspect `130e9f2` for the source-zone matrix, visibility checks, and field-replacement boundary.
