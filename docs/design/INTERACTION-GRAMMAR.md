...
| Play an Event (`[Main]`) | Drag | Hand card → **own field**; presents in spotlight (SP-1) and fizzles to trash after resolving (MO-8) | `PLAY_CARD {cardInstanceId}` | 🔧 OPT-416 (gesture) + OPT-464 (spotlight) |
...
| Redistribute DON | Drag | **DON token visual on card** → own card, then Confirm | staged → `REDISTRIBUTE_DON` | 🔧 OPT-416 (re-skin source; routing exists) |
...
| Counter (Character) | Drag | Hand card → **defending card** | `USE_COUNTER {cardInstanceId, counterTargetInstanceId}` | 🔧 OPT-416 (was trash-drop) |
| Counter (Event) | Drag | Hand card → **own field**; presents in spotlight (SP-1); defender comes from battle state | `USE_COUNTER_EVENT {cardInstanceId, counterTargetInstanceId}` | 🔧 OPT-416 amended (was trash-drop) |
| Select blocker | Click (toggle) + mid-zone **Block** | Eligible blocker | `DECLARE_BLOCKER {blockerInstanceId}` | 🔧 OPT-416 (add toggle/Escape) |
...
- **MO-5 — Reduced motion is a requirement, not a fallback.** Every preset ships its reduced path (`useReducedMotion`): loops → static full-opacity, one-shots → ≤100ms opacity change or nothing, springs → instant. The FB-2 rejection shake reduces to the static red flash only.
...
**Known current-state deviations** (owned by tickets above unless noted): counter-via-trash-drop; redistribute gold bar + phantom `--gb-accent-gold`; blocker selection lacks toggle/Escape; every opponent card lights as an attack target; slots/stage accept any card type; no affordability dim; silent rejections cleared on send; stage drop overlay is green (migrates to blue per SIG table); `[Main]` activation hidden behind right-click only; Events have no board drop surface (dragged to trash today); all zone moves share one travel animation regardless of the card's fate; **reveals are invisible** — `CARDS_REVEALED` has no UI at all and is suppressed even from the event log (rules-fidelity gap, §4).
