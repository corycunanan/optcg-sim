...
**The keyword family is a deliberate exception to the 46% step — do not "correct" it back.** `--effect-keyword` is `oklch(56% 0.125 62)`, well above the 46% row, because on a printed card an evergreen keyword is a bright orange hexagon — the loudest mark in the rules box — and reproducing that treatment is the point of the family (OPT-677). The printed hexagon also carries white lettering, and the fill is darkened far enough that white clears AA on it, so `--effect-keyword-fg` is an alias for the shared white `--effect-notation-fg` rather than an ink of its own (OPT-690; it was previously dark navy at a brighter 63% fill). This is also the only notation family that takes a shape rather than a rounded rectangle; see [SHAPE-LANGUAGE.md](SHAPE-LANGUAGE.md).

Lightness is what moves this fill's contrast pairs, so 56% is not adjustable on taste alone. `effect-notation-label/keyword` (white on the fill, 4.82:1) is the binding constraint and it only closes upward: white crosses AA at about 57% (4.62:1) and fails by 58% (4.44:1). Below roughly 54% the chip stops reading orange and starts reading rust, which is the wall on the other side. Chroma has to come *down* with lightness rather than up — the sRGB gamut edge at this hue falls from 0.145 at 63% to 0.129 at 56%, and over-asking rotates the rendered hue away from the printed orange rather than merely clipping it (`oklch(56% 0.14 62)` renders at hue 58.6, visibly redder), so 0.125 sits just inside the edge and holds hue 62 exactly. `effect-notation-keyline/keyword` (the white keyline on the fill, 4.24:1) now clears its 3:1 floor comfortably; the keyline stays because the darker fill reads 3.58:1 against the page navy rather than 4.75:1, so the frame carries more of the separation from the page than it used to. Both pairs are pinned in `scripts/contrast-pairs.json` and gated by `pnpm run check:contrast`.
