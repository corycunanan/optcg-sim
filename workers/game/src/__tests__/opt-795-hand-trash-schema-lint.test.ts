...
const cards = readdirSync(directory).filter(file=>file.endsWith(".md")).flatMap(file=>
...
describe("OPT-795 printed hand-trash schema lint",()=>{
  it("audits the complete documented card pool",()=>{
    expect(cards.map(card=>card.id).sort()).toEqual(["OP12-040","OP14-045","OP14-049","OP14-056"]);
  });
  it.each(cards)("$id rejects the inverse Life placeholder even inside compound triggers",({id,text})=>{
    const schema=structuredClone(getEffectSchema(id)!);
    expect(findHandTrashTriggerViolations(text,schema)).toEqual([]);
    const block=schema.effects.find(block=>block.trigger && "event" in block.trigger && block.trigger.event === "CARD_TRASHED_FROM_HAND")!;
  ...
  it("accepts legitimate Life text and restricts trigger-count refs to hand-trash triggers",()=>{
    ...
    expect(findHandTrashTriggerViolations("When a card is added from your Life to your hand.",schema)).toEqual([]);
    expect(validateEffectSchema(schema,schema.card_id)).toContainEqual(expect.stringContaining("__triggering_hand_trash"));
  });
});
