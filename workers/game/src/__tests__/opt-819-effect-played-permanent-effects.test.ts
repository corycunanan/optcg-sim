...
const elderCards: CardData[] = [
  ...
  ...[
    ...
    ...nusjuro,
    id: effectSchema.card_id!,
    name: effectSchema.card_name!,
    effectSchema,
  })),
];
    ...
    if (options.promptType === "OPTIONAL_EFFECT")
      ...
      state = resume(
        state,
        {
          ...
          keptCardInstanceId: kept,
          orderedInstanceIds: options.cards
            .map((c) => c.instanceId)
            .filter((id) => id !== kept),
          destination: "bottom",
        },
...
describe("OPT-819 permanent effects during nested play continuations", () => {
  it.each([
    ...
    for (const id of ids) {
      const instance = state.players[0].characters.find(
        (c) => c?.cardId === id
      )!;
      expect(instance).toBeDefined();
      const blocks = db
        .get(id)!
        .effectSchema!.effects.filter((e) => e.category === "permanent");
      expect(blocks.length).toBeGreaterThan(0);
      const records = state.activeEffects.filter(
        (e) =>
          e.sourceCardInstanceId === instance.instanceId &&
          e.category === "permanent"
      );
      expect(records).toHaveLength(
        blocks.filter((b) => b.modifiers?.length).length
      );
      for (const block of blocks) {
        const identity = (e: {
          ...
          e.sourceCardInstanceId === instance.instanceId &&
          e.sourceEffectBlockId === block.id;
        expect(records.filter(identity)).toHaveLength(
          block.modifiers?.length ? 1 : 0
        );
        expect(state.prohibitions.filter(identity)).toHaveLength(
          block.prohibitions?.length ?? 0
        );
      }
      expect(registerCardEnteredField(state, instance, db.get(id)!)).toBe(
        state
      );
    }
    ...
    for (const id of ["OP13-089", "OP13-091"]) {
      ...
      if (blocker)
        expect(hasGrantedKeyword(blocker, "BLOCKER", state, db)).toBe(true);
    }
    expect(
      runPipeline(
        state,
        {
          type: "DECLARE_ATTACK",
          attackerInstanceId: played.instanceId,
    ...
    const belowThreshold = {
      ...state,
      players: [
        { ...state.players[0], trash: state.players[0].trash.slice(0, 6) },
        state.players[1],
...
it("keeps opponent permanents when a KO-batch trigger plays an Elder batch", () => {
  const db = createTestCardDb();
  for (const data of elderCards) db.set(data.id, data);
  ...
  db.set(recruit.id, recruit);
  let state = createBattleReadyState(db);
  const victim = {
    ...card(recruit.id, "recruit"),
    owner: 1 as const,
    controller: 1 as const,
  };
  state.players[0].characters = padChars([card(source.id, "source")]);
  state.players[1].characters = padChars([
    victim,
    { ...card(CARDS.VANILLA.id, "other-victim"), owner: 1, controller: 1 },
  ]);
  state.players[1].trash = [
    "OP13-083",
    "OP13-091",
    ...Array.from({ length: 8 }, () => CARDS.VANILLA.id),
  ].map((id, i) => ({
    ...card(id, `opponent-trash-${i}`, "TRASH"),
    owner: 1,
    controller: 1,
  }));
  state = registerCardEnteredField(state, victim, recruit);
  const result = runPipeline(
    state,
    { type: "ACTIVATE_EFFECT", cardInstanceId: "source", effectId: "ko-batch" },
    db,
    0
  );
  expect(result.valid).toBe(true);
  state = drain(result.state, db);
  for (const id of ["OP13-083", "OP13-091"]) {
    const played = state.players[1].characters.find((c) => c?.cardId === id)!;
    expect(played).toBeDefined();
    expect(played.owner).toBe(1);
    expect(played.controller).toBe(1);
    expect(
    ...
    if (id === "OP13-091")
      expect(hasGrantedKeyword(played, "BLOCKER", state, db)).toBe(true);
  }
  expect(
    state.players[1].trash.filter((c) =>
      ["recruit", CARDS.VANILLA.id].includes(c.cardId)
    ).length
